...
/**
 * TB 月补偿（按结算/完结维度，MONTH_COMPLETE -> queryType=3）
...
public class TbOrderMonthByCompleteScheduler {
    ...
    public void scheduled() {
        ...
        log.info("TB month(complete) sync start: days={}, stepMinutes={}, slices={}", days, step, slices);

        LocalDateTime cursor = start;
        for (long i = 0; i < slices; i++) {
            LocalDateTime next = cursor.plusMinutes(step);
            tbOrderSyncService.syncRange(cursor, next, 2L, TbSyncType.MONTH_COMPLETE);
            cursor = next;
        }

        log.info("TB month(complete) sync done");
    }
}
