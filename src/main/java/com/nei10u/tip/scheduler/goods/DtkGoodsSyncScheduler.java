package com.nei10u.tip.scheduler.goods;
...
public class DtkGoodsSyncScheduler {
    ...
    public void cleanupExpiredCouponGoods() {
        log.info("开始执行过期优惠券商品清理任务...");
        try {
            int deleted = goodsService.cleanupExpiredCouponGoods();
            log.info("过期优惠券商品清理任务完成，删除 {} 条", deleted);
        } catch (Exception e) {
            log.error("过期优惠券商品清理任务异常", e);
        }
    }
